...
! function(a, b, c, d) {
    ...
    }, a(c).ready(function() { g("start", v), v.on(t.updateEvent, k).on(t.forceEvent, m), a(c).on(t.updateEvent, k), t.autoInit && (v.on(t.loadEvent, l), l()) })
...
$(function() {
    $('.owl-carousel').each(function() {
        ...
        if (autow != true) { autow = false; }
        if (center != true) { center = false; }
        if (drag != true) { drag = false; }
        if (ath != false) { ath = true; }
        if (margin == null) { margin = 30; }
        if (item2 == null) { item2 = item; }
        if (item3 == null) { item3 = item2; }
        if (drag == false) { $(this).owlCarousel({ loop: true, nav: nav, dots: dot, lazyLoad: true, margin: margin, autoWidth: autow, autoplay: autoplay, autoplayTimeout: 5000, autoplayHoverPause: true, autoHeight: ath, navText: ['', ''], responsive: { 0: { items: item3 }, 639: { items: item } } }); } else {
            $(this).owlCarousel({
                loop: true,
                nav: nav,
                dots: dot,
                margin: margin,
                autoWidth: autow,
                responsive: { 0: { items: item3 }, 639: { items: item } },
                lazyLoad: true,
                autoHeight: ath,
                navText: ['', ''],
                onDragged: function() {
                    var owl_active = $('.ow1').find('.owl-item.active').children().attr('class');
                    var $data_filter = owl_active.replace('item ', ''),
                        $filter = $('.btw .btf'),
                        $active_filter = $('.btw .btf[data-filter="' + $data_filter + '"]');
                    $filter.removeClass('btc');
                    $active_filter.addClass('btc');
                }
            });
        }
    });
    var heightt = Math.round($(window).height() * 0.8);
    if (heightt % 2 == 1) { heightt = heightt + 1; }
    $('.hvimg .owl-carousel img').css('max-height', heightt);
    $('.btw .btf').hover(function() {
        var $this = $(this),
            $number = $this.data('number');
        $('.btw .btf').removeClass('btc');
        $this.addClass('btc');
        $('.ow1').trigger("to.owl.carousel", [$number, 400, true]);
    });
    $('.mft .cls, .ovl').click(function() {
        $('.mft').removeClass('mftc');
        $('.ovl').removeClass('ovlc');
    });
    $('#tgn').click(function() {
        $('.mft').addClass('mftc');
        $('.ovl').addClass('ovlc');
        $('.mft img').attr('src', function() { return $(this).data('src'); });
    });
    if ($('#mn').offset().top > 0) { $('#mn').addClass('mnc'); } else { $('#mn').removeClass('mnc'); }
    $(window).scroll(function() {
        if ($('#mn').offset().top > 0) { $('#mn').addClass('mnc'); } else { $('#mn').removeClass('mnc'); }
        $('img').attr('src', function() { return $(this).data('src'); });
        $('.llbr').each(function() {
            var $br = $(this).data('br');
            $(this).css('background', $br);
        });
    });
    if ($('.usdn').outerWidth() == null) { var ht2 = 0; } else { var ht2 = $('.usdn').outerWidth(); }
    $('#mn form').css('width', $('#mn').width() - $('#mn #tgn').width() - $('#mn .l').width() - $('#mn .dn').width() - ht2 - 80);
    $('.ctqt .ys').click(function() {
        $('.ctqt .b1').css('display', 'none');
        $('.ctqt .t1').text('Cáº£m Æ¡n báº¡n Ä‘Ã£ pháº£n há»“i');
    });
    $('.ctqt .no').click(function() {
        $('.ctqt .b div').css('display', 'none');
        $('.ctqt .t1').text('Ráº¥t tiáº¿c vá» Ä‘iá»u Ä‘Ã³');
        $('.ctqt form').css('display', 'inline-block');
        $('.ctqt .t2').css('display', 'inline-block');
        $('.ctqt textarea').text('');
        $('.ctqt textarea').focus();
    });
    var submit = $(".submit-form");
    submit.click(function() {
        var data = $('form#ykien').serialize();
        var $n = $('.ykien').val();
        if ($n == '') { alert('CÃ³ váº» nhÆ° báº¡n chÆ°a nháº­p Ã½ kiáº¿n cá»§a mÃ¬nh'); } else {
            $('.ctqt form').css('display', 'none');
            $('.ctqt .t2').css('display', 'none');
            $('.ctqt .t1').text('Cáº£m Æ¡n báº¡n Ä‘Ã£ pháº£n há»“i');
            $.ajax({
                type: 'GET',
                url: 'https://script.google.com/macros/s/AKfycbzZrgbjbJk_Jhyg-YCe6ZlgNIgGh-ZVHCwO9I6NJgFQw5E9uh0/exec',
                dataType: 'json',
                crossDomain: true,
                data: data,
                success: function(data) {
                    if (data == 'false') { alert('Thao tÃ¡c khÃ´ng thÃ nh cÃ´ng má»i báº¡n kiá»ƒm tra láº¡i'); } else {
                        $('.ctqt form').css('display', 'none');
                        $('.ctqt .t2').css('display', 'none');
                        $('.ctqt .t1').text('Cáº£m Æ¡n báº¡n Ä‘Ã£ pháº£n há»“i');
                    }
    ...
    $('.src').click(function() {
        $('#mn form').addClass('formc');
        $('#mn input').focus();
    })
    $('.clsrc').click(function() { $('#mn form').removeClass('formc'); })
    $("#mn form").mouseout(function() { $('#mn form').removeClass('formc'); });
    $(document).on("keydown", function(e) { if (e.which === 8 && !$(e.target).is("input, textarea")) { e.preventDefault(); } });
    var owl = jQuery(".owl-carousel");
    $(document.documentElement).keyup(function(event) { if (event.keyCode == 37) { owl.trigger('prev.owl.carousel', [700]); } else if (event.keyCode == 39) { owl.trigger('next.owl.carousel', [700]); } });
});