...
$locs = check_string($_POST['select']);
$tab = check_string($_GET['tab']);
if(isset($_POST['select']) and $locs != ''){
$finds = "WHERE  `content` LIKE '%$locs%' or
              `title` LIKE '%$locs%'  or
               `slug` LIKE '%$locs%' ORDER BY money DESC";
}else{
$finds = " ORDER BY id DESC" ;
}
$tab = ceil($tab);
$i = 1;
$num_rec_per_page = 12;
$start_from = ($tab - 1) * $num_rec_per_page;
$start_from = ceil($start_from);
$nums = $duogxaolin->num_rows(" SELECT * FROM `blogs` $finds");
$nums = ceil($nums / $num_rec_per_page); 
?>
<!--
//////////////////////////////////////////////////////////////////////
    ...
    <main>
            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-12 col-md-8 text-center">
                        <h1 class="display-2 mb-4">Câu hỏi thường gặp</h1>
        ...
        <section class="section section-lg line-bottom-light">
            <div class="container mt-n7 mt-lg-n12 z-2">
                ...
                <?php  
                 foreach ($duogxaolin->get_list(" SELECT * FROM `blogs` $finds LIMIT $start_from, $num_rec_per_page") as $row) { ?>
                    <div class="col-12 col-md-4 mb-4">
                        <div class="card bg-white border-light shadow-soft p-4 rounded">
                            <a href="<?=$duogxaolin->home_url()?>/blogs/<?=$row['slug']?>"><img src="<?=$row['img']?>" class="card-img-top" alt="">
                            </a>
                            <div class="card-body p-0 pt-4"><a href="<?=$duogxaolin->home_url()?>/blogs/<?=$row['slug']?>" class="h3"><?=$row['title']?></a>
                                <div class="d-flex align-items-center my-4"><img class="avatar avatar-sm rounded-circle" src="<?=$duogxaolin->site('favicon',$domain)?>" alt="">
                                    <h6 class="text-muted small ml-2 mb-0">Đại Học Mở Hà Nội</h6>
                                </div>
                            </div>
                        </div>
                    </div>
           ...
           <!--             <?php if ($tab == 1) { ?>
                            ...
                            <?php } else { ?>
                             <li class="page-item"><a class="page-link" href="<?=$duogxaolin->home_url()?>/helper/<?=$tab - 1?>">Previous</a>
                            </li>
                            <?php } ?> -->
                            <?php for ($n = 1; $n <= $nums; $n++) {
                            if ($n == $tab) {
                                echo " <li class='page-item active'><a class='page-link' href='" .$duogxaolin->home_url()."/helper/". $tab . "'>" . $tab . "</a>
                                </li>";
                            } else {
                                echo "<li class='page-item'><a class='page-link' href='" .$duogxaolin->home_url()."/helper/". $n . "'>" . $n . "</a>";
                            }
                        } ?>
